...
def analysis_two_actions(a1, a2):
    ...
    print(a1, a2, cmd_list_a1, cmd_list_a2, a1 == a2)
...
def visual_action_distribution():
    ...
    expert_path = 'dqn_policy_with_tracks.npz'
    size = 100
    ...
    # Imitation Policy
    x = 40000
    # gail_policy = np.load('dqn_policy_evaluate_{}.npz'.format(x))
    gail_policy = np.load('dqn_policy_test_{}.npz'.format(x))

    gail_rew = gail_policy['rews']
    print('IP SR:', len(gail_rew) / size * 100, np.mean(gail_rew))

    dqn_acs, gail_acs = dqn_policy['acs'][:, 0], gail_policy['acs']
    ...
    for i, num in enumerate(gail_num):
        ...
        count += int(dqn_action == gail_action)
    ...
    print('Count:', count, len(gail_acs))
